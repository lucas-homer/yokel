...
- `vault-transit` — a tiny dev-mode Vault whose only job is to hold the `autounseal` transit key. It is
  the **local stand-in for cloud KMS**. `vault-transit-init` enables its transit engine + key.
...
- **Main Vault stuck sealed / `vault` Application never goes Healthy.** Almost always the transit seal:
  the `vault-transit` pod restarted and lost the in-memory autounseal key, so the main Vault can no
  longer unseal its existing raft data. There's no in-place fix (the data is encrypted under the lost
  key) — rebuild: `task dev-down && task dev-up`. (Prod doesn't have this edge; KMS is always available.)
