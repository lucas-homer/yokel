...
export async function persistReconciliation(
  ...
  const [stored] = await sql<StoredWindowRow[]>`
  ...
  await sql`
    insert into participation_windows (
      ...
      version, current_observation_ids, provenance, change_history, derived_at
    ) values (
      ${toPersist.ocd_id}, ${toPersist.fr_document_number}, ${toPersist.regs_document_id},
      ${toPersist.regs_object_id}, ${sql.json(toPersist.docket_id)}, ${toPersist.rin},
      ...
      ${toPersist.confidence}, ${sql.json(toPersist.conflict_flags)}, ${toPersist.status},
      ${toPersist.submission_url}, ${toPersist.govinfo_url}, ${sql.json(toPersist.tags)},
      ${toPersist.version}, ${sql.json(toPersist.current_observation_ids)},
      ${sql.json(toPersist.provenance)}, ${sql.json(toPersist.change_history)}, ${now.toISOString()}
    )
  ...
  if (conflict) {
    ...
    await sql`
      insert into conflict_records (
        ...
        ${conflict.source_a}, ${conflict.source_b}, ${sql.json(conflict.conflict_flags)},
