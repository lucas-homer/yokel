...
function probe(name: string, accepted: boolean, note = "") {
  out.push(
    `  ${accepted ? "ACCEPTS" : "REJECTS"}  ${name}${note ? ` — ${note}` : ""}`,
  );
}
...
const P = (o: Record<string, unknown>) =>
  ParticipationWindow.safeParse(baseWindow(o)).success;

// ── PROBE A: CONFLICTING with EMPTY conflict_flags — a verdict of "they disagree" naming no reason.
probe(
  "A) confidence='conflicting' with conflict_flags=[] (no named reason)",
  P({
...
probe(
  "B) confidence='high' + conflict_flags=['fr_regs_date_mismatch']",
...
probe(
  "C) confidence='high' + conflict_flags=['withdrawn_vs_open']",
...
probe(
  "D3) tz_normalization_only on confidence='stale'",
...
probe(
  "E1) ocd_id 'ocd-participation-window/federal/ ' (trailing space only after slash)",
  ocdProbe("ocd-participation-window/federal/ "),
  "whitespace-only tail accepted?",
);
probe(
  "E2) ocd_id with embedded newline 'ocd-participation-window/federal/x\\ny'",
  ocdProbe("ocd-participation-window/federal/x\ny"),
  "JS $ matches before \\n — second line slips?",
);
probe(
  "E3) ocd_id with a SECOND federal segment injected",
  ocdProbe("ocd-participation-window/federal/a/../../evil"),
  "path-traversal-shaped id accepted?",
);
probe(
  "E4) makeOcdId with a frDocNum containing a slash",
  (() => {
    try {
      return OcdId.safeParse(makeOcdId({ frDocNum: "2025-01/../x" })).success;
    } catch {
      return false;
    }
  })(),
...
probe(
  "F1) resolved_close_utc with a -05:00 offset (not Z) under HIGH",
...
probe(
  "F2) resolved_close_utc date-only '2018-12-21' under HIGH",
...
probe(
  "G) ConflictRecord with conflict_flags=['tz_normalization_only'] ONLY",
...
probe(
  "H) ConflictRecord with source_a === source_b (same source)",
  ConflictRecord.safeParse({
    ocd_id: makeOcdId({ frDocNum: "2018-27875" }),
    observation_a_id: "o1",
    observation_b_id: "o1",
...
probe(
  "I) duplicate conflict_flags entries accepted",
  P({
    confidence: "medium",
    conflict_flags: ["tz_normalization_only", "tz_normalization_only"],
  }),
...
probe(
  "J) status='withdrawn' + confidence='high' + a real resolved_close_utc",
  P({
    confidence: "high",
    status: "withdrawn",
    resolved_close_utc: "2025-01-01T00:00:00.000Z",
  }),
