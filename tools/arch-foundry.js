export const meta = {
  ...
  description:
    "Design DocketClock architecture (vs Codex plan) and Watershed Watch architecture on top of it, via competing architects + adversarial critique + Opus synthesis",
  phases: [
    {
      title: "DocketClock design",
      detail: "3 architects propose competing DocketClock architectures",
    },
    {
      title: "DocketClock review",
      detail: "adversarial critique of each proposal",
    },
    {
      title: "DocketClock synthesis",
      detail: "Opus lead-architect synthesizes final design + Codex ledger",
    },
    {
      title: "Watershed fit",
      detail: "should Watershed Watch build on DocketClock at all?",
    },
    {
      title: "Watershed design",
      detail: "thin-client vs thick-vertical architecture proposals",
    },
    { title: "Watershed review", detail: "adversarial critique of each" },
    {
      title: "Watershed synthesis",
      detail: "Opus synthesizes final Watershed Watch architecture",
    },
  ],
};
  ...
  properties: {
    ...
    canonical_model: {
      ...
      items: {
        ...
        required: ["object", "key_fields", "why"],
      },
    },
    pipeline_stages: {
      type: "array",
...
const DC_SYNTH = {
  ...
  properties: {
    ...
    canonical_object: {
      ...
      properties: {
        ...
        key_fields: {
          ...
          items: {
            type: "object",
            properties: { field: { type: "string" }, desc: { type: "string" } },
            required: ["field", "desc"],
          },
        },
        design_notes: { type: "string" },
      },
      required: ["name", "key_fields", "design_notes"],
    },
    pipeline: {
      type: "array",
      items: {
        ...
        states: {
          ...
          items: {
            ...
            properties: {
              state: { type: "string" },
              meaning: { type: "string" },
              alert_behavior: { type: "string" },
            },
            required: ["state", "meaning", "alert_behavior"],
          },
        },
        how_computed: { type: "string" },
      },
      required: ["states", "how_computed"],
    },
    delivery_surfaces: {
      type: "array",
      items: {
        type: "object",
        properties: { surface: { type: "string" }, detail: { type: "string" } },
    ...
    mvp_boundary: {
      ...
      required: ["in_scope", "deferred", "first_customer"],
    },
    codex_ledger: {
      type: "object",
      properties: {
    ...
    divergences_from_prior_dossier: {
      type: "array",
      items: { type: "string" },
    },
    build_sequence: {
...
const FIT = {
  type: "object",
  properties: {
    builds_on_docketclock: {
      type: "string",
      enum: ["yes-fully", "yes-partially", "no"],
    },
    ...
    layer_boundary: {
      type: "object",
      properties: {
        docketclock_provides: { type: "array", items: { type: "string" } },
        watershed_adds: { type: "array", items: { type: "string" } },
      },
      required: ["docketclock_provides", "watershed_adds"],
    },
  ...
  required: [
    ...
    "wedge_reasoning",
    "layer_boundary",
    "requirements_on_docketclock",
    "counterfactual_if_standalone",
    "risks",
  ],
};
...
const WW_PROPOSAL = {
  ...
  properties: {
    ...
    action_loop: {
      ...
      items: {
        type: "object",
        properties: { step: { type: "string" }, what: { type: "string" } },
        required: ["step", "what"],
      },
    },
    coverage_tiers: {
  ...
  properties: {
    ...
    mvp_boundary: {
      ...
      properties: {
        in_scope: { type: "array", items: { type: "string" } },
        deferred: { type: "array", items: { type: "string" } },
        first_partner: { type: "string" },
      },
      required: ["in_scope", "deferred", "first_partner"],
    },
    build_sequence: {
      type: "array",
      items: {
...
const aResults = await pipeline(
  A_STANCES,
  (s) =>
    agent(proposePrompt(s), {
      label: `dc-propose:${s.key}`,
      phase: "DocketClock design",
      model: "sonnet",
      schema: DESIGN_PROPOSAL,
    }),
  (prop, s) =>
    prop
      ? agent(
...
${JSON.stringify(prop)}`,
          {
            label: `dc-critique:${s.key}`,
            phase: "DocketClock review",
            model: "sonnet",
            schema: CRITIQUE,
          },
        ).then((c) => ({ stance: s.key, proposal: prop, critique: c }))
      : null,
);
...
const bResults = await pipeline(
  B_STANCES,
  (s) =>
    agent(wwProposePrompt(s), {
      label: `ww-propose:${s.key}`,
      phase: "Watershed design",
      model: "sonnet",
      schema: WW_PROPOSAL,
    }),
...
${JSON.stringify(prop)}`,
          {
            label: `ww-critique:${s.key}`,
            phase: "Watershed review",
            model: "sonnet",
            schema: CRITIQUE,
          },
        ).then((c) => ({ stance: s.key, proposal: prop, critique: c }))
